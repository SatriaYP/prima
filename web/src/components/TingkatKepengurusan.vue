<template>
  <div class="tingkat-kepengurusan">
    ...
    <div v-else-if="levels.length === 0" class="empty">Belum ada tingkat kepengurusan</div>
    <div v-else>
        ...
        <tbody>
          <tr v-for="level in levels" :key="level.id">
            ...
            <td>
              ...
              <button class="btn btn-sm btn-red" @click="confirmDeleteLevel(level)">Hapus</button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>
    <TingkatFormDialog v-if="showForm" :level="selectedLevel" @close="closeForm" @saved="onLevelSaved" />
    ...
    <div v-if="showDeleteDialog" class="dialog-content dialog-confirm">
      ...
      <p>Hapus tingkat <b>{{ levelToDelete?.name }}</b>?</p>
...
export default {
  ...
  setup() {
    ...
    return {
      levels, loading, error, showForm, selectedLevel,
      openForm, closeForm, onLevelSaved,
      confirmDeleteLevel, showDeleteDialog, levelToDelete, closeDeleteDialog, deleteLevelConfirmed,
...
.levels-table { width: 100%; border-collapse: collapse; margin-bottom: 12px; }
.levels-table th, .levels-table td { padding: 8px 10px; border-bottom: 1px solid #eee; }
.levels-table th { background: #f7f7f7; font-weight: 600; }
